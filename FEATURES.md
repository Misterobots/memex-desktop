...
- Dependency audit: production dependencies report 0 vulnerabilities after the `js-yaml` 4.3.1 lockfile update. The safe lockfile refresh reduced the development audit to 12 findings (1 moderate, 10 high, 1 critical); all remaining fixes require major Electron/electron-builder/Vite upgrades.
- Security-upgrade trial: Electron 41.10.3, electron-builder 26.15.3, Vite 6.4.3, PostCSS 8.5.26, and shell-quote 1.10.0 reached 0 audit findings and passed tests/typechecks under the bundled Node 24.19.0 runtime, but packaging could not rebuild `node-pty` without Visual Studio Spectre-mitigated libraries; the candidate was reverted and the current release baseline remains unchanged.
