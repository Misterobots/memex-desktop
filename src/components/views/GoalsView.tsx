  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex h-10 flex-shrink-0 items-center gap-1 border-b border-border/60 bg-surface px-4">
        <button onClick={() => setSection("work")} className={`rounded-md px-2.5 py-1 text-xs transition-colors ${section === "work" ? "bg-surface2 text-text" : "text-muted hover:text-text"}`}>Threads</button>
        <button onClick={() => setSection("schedule")} className={`rounded-md px-2.5 py-1 text-xs transition-colors ${section === "schedule" ? "bg-surface2 text-text" : "text-muted hover:text-text"}`}>Schedules</button>
      </div>
