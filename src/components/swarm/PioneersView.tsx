...
export function PioneersView({ events, active, workspaceKey = "global" }: { events: MessageEvent[]; active: boolean; workspaceKey?: string }) {
  ...
  return <>
    <aside aria-label="Pioneers" style={{ width: panelWidth }} className={`relative flex h-full shrink-0 overflow-hidden border-l border-border/60 bg-surface ${resizing ? "select-none" : "transition-[width] duration-300"}`}>
