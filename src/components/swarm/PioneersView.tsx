...
export function PioneersView({ events, active, workspaceKey = "global" }: { events: MessageEvent[]; active: boolean; workspaceKey?: string }) {
  ...
  if (dockMode) return <div aria-label="Agent Dock" className="fixed bottom-3 right-3 z-30 w-[min(520px,calc(100vw-1.5rem))] rounded-xl border border-border/60 bg-surface/95 p-3 shadow-2xl backdrop-blur"><div className="mb-2 flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${active ? "animate-pulse bg-accent" : "bg-emerald-400"}`} /><span className="text-xs font-bold uppercase tracking-widest text-text">AgentDock</span><span className="text-[10px] text-faint">{liveWorkers.length} active</span><div className="ml-auto flex items-center gap-2"><button type="button" onClick={() => setDockMode(false)} className="rounded border border-accent/40 px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-accent hover:bg-accent/10">Full panel</button><button type="button" onClick={() => setPanelOpen(false)} className="text-faint hover:text-text" aria-label="Hide Pioneers">×</button></div></div>{liveWorkers.length ? <div className="flex gap-2 overflow-x-auto">{liveWorkers.map((worker) => <button key={worker.worker_id} onClick={() => { setSelected(worker.worker_id); setDockMode(false); }} className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-border/60 bg-surface2 px-2 py-2 text-left hover:border-accent/50"><Portrait worker={worker} /><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-bold text-text">{worker.pioneer_name}</span><span className="mt-1 block truncate text-[9px] text-muted">{worker.activities.at(-1)?.text ?? worker.task}</span></span></button>)}</div> : <div className="py-2 text-[10px] text-faint">No agents are actively running.</div>}</div>;
  return <>
