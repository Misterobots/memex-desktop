...
export function AccountMenu() {
  ...
  const connectionLabel = connected ? "Connected" : "Runtime unavailable";
    ...
    <div ref={root} className="relative">
        ...
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-64 rounded-xl border border-border/70 bg-surface2 p-2 shadow-2xl">
          <div className="px-2 py-1.5">
            ...
            <div className={`mt-0.5 text-xs ${connected ? "text-green" : "text-muted"}`}>
              {connected ? "Connected to Memex runtime" : "Memex runtime unavailable"}
            </div>
          </div>
          ...
          <button onClick={() => void bridge?.health.check()} className="w-full rounded-lg px-2 py-2 text-left text-sm text-accent hover:bg-accent/10">Check connection</button>
        </div>
      )}
      <button onClick={() => setOpen((value) => !value)} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-surface2">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className={`block text-[11px] ${connected ? "text-green" : "text-muted"}`}>{connectionLabel}</span>
        </span>
