...
- **The same tests cannot be run in the container either.** The image does have `ollama` and `prometheus_client`, but `/app` contains only `agents`, `comfy_io` and `entrypoint.sh` — `tests/` is not mounted. So today these guards run only where the full dependency set is installed on the host. Worth deciding deliberately: mount or copy `tests/` into the image, or accept that the Gauntlet guard is workstation-only and therefore depends on the two-name fix above being applied.
