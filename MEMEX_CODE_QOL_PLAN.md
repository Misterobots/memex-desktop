...
3. The runtime half differs by tree. In `Memex_Core` — the one `agent_runtime` actually mounts for this client — DevHarness is guarded by `_is_swarm_turn`, which exempts `swarm_mode`, so a Collective from Code *did* reach the coordinator; it simply never carried `research_mode`, so it took the coordinator's build path and produced no perspective matrix. In `Agent_Swarm` the gate is only `dev_mode and stream and not gauntlet_mode`, so `swarm_mode` and `research_mode` are both discarded before routing and the request never leaves DevHarness. Either way nothing in the transcript revealed the mismatch, because the desktop sends flags rather than a mode string.

Implemented (desktop commits `29990ad`, `8135d3a`, `9f80159`, `12b0eee` on `fix/pioneers`; runtime commit `9c9ce26` on `feat/code-gateway-gpu-arbitration`):
...
- **Runtime attribution, corrected after `docker inspect`:** both `agent_runtime` (`:8008`) and `agent_runtime_dev` (`:8009`) on this machine bind-mount `Memex_Core\agents` at `/app/agents`, and `uvicorn agents.main:app` runs without `--reload`. Nothing running mounts `Agent_Swarm/agents`. The `Agent_Swarm` edit is therefore correct-in-principle but **inert** for the Localhost and Home LAN profiles until that tree is deployed; the public "Memex Anywhere" profile is a separate deployment that was not inspected.
- **The desktop half did not depend on the runtime port at all.** `Memex_Core`'s DevHarness guard has always exempted `swarm_mode` (formerly via `_is_swarm_turn`, now `_routes_to_dev_harness`), so a Collective launched from Code reaches `church.py` → `COORDINATE` → `handlers/coordinate.py` → `orchestrator.py` Perspective Research Mode as soon as the client sends the second flag. The port was needed for Gauntlet, not for Perspectives.
- No perspective matrix has been watched end to end from either workspace, from either profile.
- Entitlement check on the live runtime, read-only `GET /api/v1/permissions` as `misterobots`: `research`, `code` and `chat` are all `true`, so Collective's new `research_mode` flag adds no access risk here. Note the two trees differ: `Memex_Core`'s `_enforce_chat_features` never requires a `swarm` feature and its `FEATURES` catalogue has no such key, while `Agent_Swarm` requires `swarm` and does list it. A Collective launched against a hosted profile whose policy denies `research` would 403 with `Feature access denied: research`.
- **Profile in use on this machine, read from `%APPDATA%\memex-desktop\config.json`:** `active` = `home-lan` → `http://192.168.2.101:8008`, default model `qwen3.8:27b`. `192.168.2.101` is this host, so that endpoint is the same `agent_runtime` container that bind-mounts `Memex_Core\agents` — i.e. the desktop's live pipeline decisions are made by `Memex_Core`, not `Agent_Swarm`. A fourth profile id exists in the file alongside the three seeds, so the profile list is user-extensible; none of the values printed here are secrets beyond the model name.
- The `Agent_Swarm` change was committed on 2026-09-22 as `9c9ce26` ("fix: let a coordinator request outrank the DevHarness preemption") on branch `feat/code-gateway-gpu-arbitration`, in that repo's own working tree. It is **not** what serves this client — `agent_runtime` mounts `Memex_Core` — and `Memex_Core` now carries the same helper independently (`_routes_to_dev_harness`, `_is_swarm_turn` retired), so the two trees agree on routing shape again. `9c9ce26` still matters only if `Agent_Swarm` is deployed somewhere, e.g. the un-inspected public `memex-anywhere` backend; otherwise it is correct code on the inactive side of the split.
- ~~`Memex_Core`'s guard exempts `swarm_mode` but not `research_mode`.~~ **Closed 2026-09-23:** the guard is now `_routes_to_dev_harness()` (`Memex_Core/agents/main.py:2466`, used at `:2546`) and its conditions include `and not request.research_mode` (`:2497`). Covered by a named section of `Memex_Core/tests/test_gauntlet_routing.py`.
- ~~Gauntlet is not implemented in the tree that serves this client.~~ **Closed 2026-09-23** in source: the port landed on `fix-sse-events` (latest commit `10d3c128`, arrived as a fast-forward merge of `worktree-keen-elm-b2e5f3`). `gauntlet_bar`/`gauntlet_handoff` fields and the missing-bar `422` (`main.py:2508-2522`), `_gauntlet_prompt` (`:697`), the immutable-contract block (`:2705-2720`), `chat_swarm(gauntlet_bar=...)` (`:2768`), the ctx hand-off through `church.py:491`/`:1344` into `handlers/coordinate.py:36`/`:130`, the critic verdict + repair + re-check in `coordination/orchestrator.py:1658-1797`, and the persistence in `swarm_run_store.py` (`:95` column, `:115` table, `:287` writer). **Live in the serving process as of 2026-09-23 — two notes remain:**
- **The port is live.** `agent_runtime` reports `running`, started `2026-09-23T13:10:37Z` — roughly ten hours *after* the merge that wrote the sources at `02:57:15Z` — still bind-mounted from `Memex_Core\agents`, still without `--reload`. Independent confirmation that the serving process holds the new code: its own `/openapi.json` `ChatRequest` now exposes 37 fields including `gauntlet_bar` and `gauntlet_handoff`, neither of which was declared before the port. The persistence half is real too — a read-only query through the store's own `_db()` finds `swarm_gauntlet_reviews` and `swarm_runs.gauntlet_bar`, so `record_gauntlet_review()` will not silently no-op. The verdict predicate was checked independently over 8 adversarial inputs with zero mismatches, including the trap case where a critic prompt merely *mentions* `VERDICT: PASS`: `(?:^|\n)[ \t]*VERDICT:[ \t]*PASS\b` correctly refuses it. No chat request was sent, so nothing spent GPU or wrote a run row.
- **The 43 new tests do not collect on a bare workstation, for a two-name reason.** Both `Memex_Core/tests/test_gauntlet_critic_gate.py` and `test_gauntlet_routing.py` gate their stubbing on `_ALLOWED_ABSENT`, which lists 12 of the 14 container-only modules the import chain needs; `ollama` (via `coordination/decomposer.py`) and `prometheus_client` (via `agents/metrics.py`) are absent, so collection aborts rather than silently skipping. Proven benign rather than assumed: supplying exactly those two stubs from outside the repo (a `-p` plugin; nothing written into the tree) makes both files collect and **all 43 pass in 2.86s**. The fix is the two names.
- ~~The same tests cannot be run in the container either.~~ **My claim was wrong and is corrected:** the runtime image mounts the *whole* repo at `/workspace` (`docker exec agent_runtime ls /workspace` shows `agents`, `tests`, `pytest.ini`, plus the plan docs), so the tests are reachable in-container after all — what the image lacks is the test runner (`python -m pytest` → `No module named pytest`). So this is not a mount problem and needs no compose change; it is a "does anything execute them" problem, and today nothing does: `.github/workflows/` holds only `docs.yml`. The guards run on the host (43 passed after the two-name fix) or in any environment that installs pytest; putting pytest into the production runtime image would be the wrong fix.

## 0.1.109 — residency duplicate-row fix (2026-09-23)

Reported by the user: three `nomic-embed-text` models resident on Lovelace, reloading after every unload. Verified state: `docker exec ollama_gpu ollama ps` lists **one** model — `nomic-embed-text:latest 0a109f422b47 323 MB 100% GPU 2048`, `expires_at` ~59 min out. Probing each address the client uses showed `localhost`, `127.0.0.1` and `192.168.2.101` all returning that same single load and `192.168.2.103` (Turing) returning none. So the three rows were one model seen through three addresses of the same daemon: `ollama:getLoadedModels` sweeps four addresses and deduped on `` `${m.name}@${base}` ``, a key that cannot collapse aliases. My defect, introduced with the VRAM indicator in `da9e460`.

Fixed in `7235f11` by extracting `electron/ollama-residency.ts` and keying on load identity instead (name, model digest, `expires_at`), which folds aliases of one daemon while keeping the same model genuinely resident on two daemons as two rows — the inverse failure mode of the old key. Four tests in `electron/__tests__/ollama-residency.test.ts` cover both directions. Typecheck clean, 176 tests / 38 files pass.

Packaged: `release/Memex Desktop Setup 0.1.109.exe`, 84,196,821 bytes, SHA-256 `8912531299b774f51dfffc65e619712e066358bfb34f2a61f7c125c3e5d2287a`, built at commit `14b0d6c`; `release/latest.yml` declares version 0.1.109 and its SHA-512 matches the value recomputed from the file. Bundle verification was done on `dist-electron/main.js`, the file actually packaged: the old `}@${` key form appears **0 times**, and the new key survives minification as ``` `${t.name}|${t.model??t.name}|${t.expires_at??""}` ```. Probing the whole `app.asar` for those markers is inconclusive on its own — `}|${` and `}@${` both occur in unrelated code (semver ranges) — which is why the main-process bundle is the right target. Content probes on the asar do confirm the mode contract is still shipped: `["code","swarm","gauntlet","plan","think"]`, `code:{dev_mode:!0}`, `swarm_mode:!0,research_mode:!0`, the slash-notice copy and the "Code with Memex" heading.

Still true and still unverified: **installed acceptance**. Neither build has been run from the installer, and no perspective matrix or Gauntlet critic verdict has been observed in a live stream.

Unrelated observation, recorded rather than chased: the reload-after-unload the user saw is expected behaviour while something calls `POST /api/embed` with a five-token input roughly every 30 seconds (25 calls in 40 minutes, timestamps on `:00`/`:30`) and `ollama_gpu` runs `OLLAMA_KEEP_ALIVE=1h` — each embed re-pins the model, so `keep_alive: 0` is undone within half a minute. Not the desktop: it issues only `/api/ps` and `/api/generate`. Not MemPalace `/health`, which is a bare `SELECT 1`. Attribution stopped there deliberately; the caller was not identified.
