...
- **The desktop half does not depend on that deploy.** `Memex_Core` guards DevHarness with an `_is_swarm_turn` check that already exempts `swarm_mode`, so a Collective launched from Code (`dev_mode` + `swarm_mode` + `research_mode`) reaches `church.py` → `COORDINATE` → `handlers/coordinate.py` → `orchestrator.py` Perspective Research Mode against the container running today. Established by reading that chain, not by observing a stream.
...
- The `Agent_Swarm` change was committed on 2026-09-22 as `9c9ce26` ("fix: let a coordinator request outrank the DevHarness preemption") on branch `feat/code-gateway-gpu-arbitration`, in that repo's own working tree. It remains inert for the `home-lan` and `localhost` profiles because the mounted tree is `Memex_Core`, where `_is_swarm_turn` already exempts `swarm_mode` — so Collective from Code reaches the coordinator without it, and only a `research_mode`-without-`swarm_mode` request still needs that gate.
...
- ~~Gauntlet is not implemented in the tree that serves this client.~~ **Closed 2026-09-23** in source: the port landed on `fix-sse-events` (latest commit `10d3c128`, arrived as a fast-forward merge of `worktree-keen-elm-b2e5f3`). `gauntlet_bar`/`gauntlet_handoff` fields and the missing-bar `422` (`main.py:2508-2522`), `_gauntlet_prompt` (`:697`), the immutable-contract block (`:2705-2720`), `chat_swarm(gauntlet_bar=...)` (`:2768`), the ctx hand-off through `church.py:491`/`:1344` into `handlers/coordinate.py:36`/`:130`, the critic verdict + repair + re-check in `coordination/orchestrator.py:1658-1797`, and the persistence in `swarm_run_store.py` (`:95` column, `:115` table, `:287` writer). **Still open — none of it is running yet:**
- **Docker Desktop is down, so `:8008` serves nothing.** `docker inspect agent_runtime` fails with `failed to connect to the docker API at npipe:////./pipe/dockerDesktopLinuxEngine`, and TCP connects to `127.0.0.1:8008`, `::1:8008` and `192.168.2.101:8008` all return `ECONNREFUSED`. Earlier the same day that port answered `GET /api/v1/health/nodes` with 200 and a Lovelace node list, so the execution plane is off rather than degraded. Two consequences: the desktop client on the `home-lan` profile has no backend at all, and because uvicorn runs without `--reload` the port cannot go live by editing files — it needs the container back up. Gauntlet acceptance is therefore waiting on the host, not on a restart command.
- **The 43 new tests cannot collect on a bare workstation, for a two-name reason.** Both `Memex_Core/tests/test_gauntlet_critic_gate.py` and `test_gauntlet_routing.py` gate their stubbing on an `_ALLOWED_ABSENT` frozenset that lists `pynvml` and the `phi.*` modules but omits `ollama` (reached from `coordination/decomposer.py`: `from ollama import Client`) and `prometheus_client` (reached from `agents/metrics.py`). Collection aborts with `ModuleNotFoundError: No module named 'ollama'` and `... 'prometheus_client'` — the same "a guard that cannot run protects nothing" mode already recorded against the Agent_Swarm Gauntlet tests. Proven benign rather than assumed: supplying exactly those two stubs from outside the repo (a `-p` plugin; nothing written into the tree) makes both files collect and **all 43 pass in 2.86s**. The fix is the two names in each `_ALLOWED_ABSENT`; left unapplied because `Memex_Core` is another hand's active checkout.
