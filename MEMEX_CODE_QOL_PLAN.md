...
- `Memex_Core`'s guard exempts `swarm_mode` but not `research_mode`, so a `research_mode`-only request that also carries `dev_mode` still becomes a Code loop there. Left untouched: no picker path produces that combination, and `Memex_Core/agents/church.py` was modified by another hand mid-session (`mtime 2026-09-22T20:44Z`), so editing it now risks colliding with in-flight work.
- **Gauntlet is not implemented in the tree that serves this client.** `Memex_Core` reads `gauntlet_mode` only to mark a turn as not-Code; `gauntlet_bar` and `gauntlet_handoff` appear nowhere in it, so the quality bar that the composer requires and the desktop-owned contract packet are accepted by the model and then dropped. The bar enforcement, `_gauntlet_prompt`, the 422, and `record_gauntlet_review` exist only in `Agent_Swarm`. Which of the two trees is authoritative is an open decision for the user, and Gauntlet's described behaviour cannot be accepted as working until it is resolved.
